            ...
            if (isset($_GET["status"])) {
              if ($_GET["status"] == "new-success") {
                echo '<div class="alert txt-success border-success outline-2x alert-dismissible fade show alert-icons" role="alert">
                    <p><b> Selamat! </b>Hasil printing berhasil di input ke database.</p>
                    <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>';
              } else if ($_GET["status"] == "error") {
                echo '<div class="alert txt-danger border-danger outline-2x alert-dismissible fade show alert-icons" role="alert">
                    <p><b> Error! </b>Terjadi kesalahan saat penyimpanan hasil printing ke database.</p>
                    <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>';
              }
            }
            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    while ($rowDetail = mysqli_fetch_assoc($resultDetail)) {
                        ...
                        if($rowDetail["FlowOut"] > 0){
                            $qtyOut = $rowDetail["FlowOut"];
                        }
                        $matetialcd = $rowDetail["MaterialCD"];
                        $qtyIn = $rowDetail["FlowIn"];
                    }
                    echo '  </tbody>
                        </table>
                    </div>
                ...
                if($qtyIn <= 0){
                ?>
                <button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#exampleModalCenter1">Input Hasil Printing</button>
                <div class="modal fade" id="exampleModalCenter1" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenter1" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-body"> 
                            <div class="modal-toggle-wrapper">  
                                <h4 class="text-center pb-2">Silahkan isi jumlah total fan hasil printing!</h4>
                                <br>
                                <form class="row g-3 needs-validation" method="POST" action="../Process/inputHasilFanPrint.php">
                                    <input type="hidden" name="mutationid" value="<?php echo $mut["MutationID"] ?>">
                                    <input type="hidden" name="materialcd" value="<?php echo $matetialcd ?>">
                                    <div class="col-md-12">
                                        <input class="form-control digits" id="validationCustom01" type="number" name="qtyFanPrint" max="<?php echo $qtyOut; ?>" placeholder="Max. <?php echo number_format($qtyOut, 0, ',', '.'); ?>" required="">
                                        <button class="btn btn-primary" style="margin-top: 10px;" type="submit">Simpan</button>
                                    </div>
                                </form>
                            </div>
                        </div>
