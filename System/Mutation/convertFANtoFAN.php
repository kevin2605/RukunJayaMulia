...
<head>
  ...
  <script>
    function getStockFan(mat){
        var material = mat.value.split(" - ");
        var materialcd = material[0];
        
        $.ajax({
            type: "POST",
            url: "../Process/getStockFan.php",
            data: "cd=" + materialcd,
            success: function (result) {
                var res = JSON.parse(result);
                $.each(res, function (index, value) {
                    document.getElementById("qtyStok").value = value.StockQty;
                });
            }
        });
    }

    function getSODetail(parm){
      var id = parm.value.split(" - ")[0];
      var logo = parm.value.split(" - ")[2];
      ...
      $.ajax({
          ...
          success: function (result) {
              ...
              $.each(res, function (index, value) {
                ...
                document.getElementById("jumlahpesanan").value = value.Quantity;
              });
          }
      });
    }
    ...
    function viewMut(x) {
      document.location = "view-mutation-FTF.php?id=" + x.value;
    }
          ...
          <div class="page-title">
            ...
            <div class="row">
              <div class="col-sm-6 ps-0">
                <h3>KONVERSI FAN KE FAN PRINT</h3>
              </div>
              <div class="col-sm-6 pe-0">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item">Fan ke Fan Print</li>
                </ol>
              </div>
            </div>
          </div>
                  ...
                  <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                      <div class="modal-header">
                        <p class="mt-1 f-m-light">Silahkan pilih fan yang akan dimutasi.</p>
                      </div>
                            ...
                            <div class="col-md-12 col-xs-12">
                              <div class="tab-content" id="ver-pills-tabContent">
                                <div class="tab-pane fade show active" id="PS12PREM" role="tabpanel"
                                  ...
                                  <div class="card">
                                    <div class="card-body custom-input">
                                      <form action="../Process/createMutFTF.php" method="POST">
                                        <div class="row g-3">
                                            <div class="col-4">
                                                <div class="col-12">
                                            ...
                                            <div class="col-8">
                                              <div class="row">
                                                <div class="col-12">
                                                    <label class="form-label" for="sales">Pilih Sales Order Printing<span style="color:red;">*</span></label>
                                                    <input class="form-control" id="sales" name="sales" list="salesOptions" onchange="getSODetail(this)" placeholder="-- Pilih SO --" required>
                                                    <datalist id="salesOptions">
                                                        <?php
                                                        $query = "SELECT so.SalesOrderID, c.CustName, so.Logo 
                                                                  FROM salesorderheader so, customer c
                                                                  WHERE so.Logo != ''
                                                                        AND so.CustID=c.CustID
                                                                        AND so.Finish = 0";
                                                        $result = mysqli_query($conn, $query);
                                                        while ($row = mysqli_fetch_array($result)) {
                                                            echo '<option value="' . $row["SalesOrderID"] . ' - ' . $row["CustName"] . ' - ' . $row["Logo"] . '"></option>';
                                                        }
                                                        ?>
                                                    </datalist>
                                                </div>
                                                ...
                                                <div class="col-4">
                                                    ...
                                                    <input class="form-control" id="Logo" name="Logo" type="text"  placeholder="-" readonly>
                                                </div>
                                                <div class="col-4">
                                                    <label class="form-label" for="jumlahpesanan">Jumlah Pesanan</label>
                                                    <input class="form-control" id="jumlahpesanan" name="jumlahpesanan" type="text" placeholder="-" readonly>
                                                </div>
                                              </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row g-3">
                                          <div class="col-5">
                                            <label class="form-label" for="productOne">FAN</label>
                                            <input class="form-control" id="productOne" name="productOne" list="fanOptions" onchange="getStockFan(this)" placeholder="-- Pilih FAN --" required>
                                            <datalist id="fanOptions">
                                                <?php
                                                $query = "SELECT MaterialCD, MaterialName FROM material WHERE UnitCD_2='FAN' AND StockQty > 0";
                                                $result = mysqli_query($conn, $query);
                                                while ($row = mysqli_fetch_array($result)) {
                                          ...
                                          <div class="col-2">
                                            <label class="form-label" for="qtyStok">Stok Fan</label>
                                            <input class="form-control digits" id="qtyStok" name="qtyStok" type="number" readonly>
                                          </div>
                                          <div class="col-2">
                                            <label class="form-label" for="qtyOut">Fan Keluar <span style="color:red;">*</span></label>
                                            <input class="form-control digits" id="qtyOut" name="qtyOut" type="number" required>
                                          </div>
                                        </div>
                                        <br><br><br>
                                        ...
                                        <div class="row g-3">
                                          <div class="col-5">
                                            <label class="form-label" for="productRes">FAN</label>
                                            <input class="form-control" id="productRes" name="productRes" list="fanPrintOptions" placeholder="-- Pilih FAN --" required>
                                            <datalist id="fanPrintOptions">
                                                <?php
                                                $query = "SELECT MaterialCD, MaterialName FROM material WHERE MaterialCD LIKE '%FH%'";
                                                $result = mysqli_query($conn, $query);
                                                while ($row = mysqli_fetch_array($result)) {
                                                    echo '<option value="' . $row["MaterialCD"] . ' - ' . $row["MaterialName"] . '"></option>';
                                                }
