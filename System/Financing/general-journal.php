    ...
    function editJournal(x) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          document.location = "edit-general-journal.php?gjid=" + x.value;
        }
      });
    }
                      ...
                      <div class="card-body">
                        <div class="row">
                          <div class="col-md-12">
                            ...
                            ?>
                            <button class="btn btn-outline-primary" type="button" <?php echo $canCreate ? 'data-bs-toggle="modal" data-bs-target=".modal-jurnal-umum"' : 'disabled'; ?>><i
                              ...
                              <div class="table-responsive custom-scrollbar user-datatable">
                                <table class="display" id="basic-100">
                                  ...
                                  <tbody>
                                    ...
                                    $query_access = "SELECT genjournal FROM useraccesslevel WHERE UserID = '$creator'";
                                    $result_access = mysqli_query($conn, $query_access);
                                    $can_update = false;
                                    if ($result_access) {
                                      $row_access = mysqli_fetch_assoc($result_access);
                                    ...
                                    while ($row = mysqli_fetch_array($result)) {
                                      ...
                                      if ($can_update) {
                                        echo '<button onclick="editJournal(this)" type="button" class="light-card border-warning border b-r-10" value="' . $row["GenJourID"] . '"><i class="icon-pencil-alt txt-warning"></i></button>';
                                      }
