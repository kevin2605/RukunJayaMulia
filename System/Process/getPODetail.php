...
if ($_POST["category"] == "BB") {
    ...
    $kueri = "SELECT pod.ItemCD, s.GoodsName, pod.Quantity, pod.UnitCD, s.UnitCD_2, pod.QuantityReceived
FROM purchaseorderdetail pod
LEFT JOIN supportinggoods s ON pod.ItemCD = s.GoodsCD
WHERE pod.PurchaseOrderID ='" . $_POST["id"] . "' AND pod.Quantity > pod.QuantityReceived;";
    $hasil = mysqli_query($conn, $kueri);
    while ($row = mysqli_fetch_array($hasil)) {
        $rows[] = $row;
    }
