...
$queryh = "INSERT INTO `mutationfanheader` (`MutationID`, `CreatedOn`, `CreatedBy`, `Description`, `CategoryCD`) 
           VALUES ('$mufid', '$datetime', '$creator', '$desc', 'MASLB')";
...
if ($resulth) {
    ...
    if ($insertSuccess) {
        ...
        header("Location:../Mutation/mutationASMLB.php?status=new-success");
    } else {
        logAction($conn, $creator, 'Create', 'gagal menambahkan mutasi konversi', 1, $mufid);
        header("Location:../Mutation/mutationASMLB.php?status=error");
    }
} else {
