...
if (isset($_GET['ReceptionID'])) {
    ...
    $queryHeader = "SELECT * FROM receptionheader WHERE ReceptionID = '$receptionID'";
    ...
    // Query untuk mengambil data detail dengan nama item
    if($header["CategoryCD"] == "BB"){
        ...
        SELECT 
            ...
            rd.documentimage_2
        FROM receptiondetail rd
        LEFT JOIN supportinggoods p ON TRIM(rd.ItemCD) = TRIM(p.GoodsCD) 
        WHERE rd.ReceptionID = '$receptionID'";
        ...
        SELECT 
            ...
            rd.documentimage_2
        FROM receptiondetail rd
        LEFT JOIN sparepart p ON TRIM(rd.ItemCD) = TRIM(p.PartCD) 
        WHERE rd.ReceptionID = '$receptionID'";
    }
    ...
    $pdf->SetXY(15, 15);
    $pdf->Cell(0, 10, 'PT. INDOPACK MULTI PERKASA                    Nota Penerimaan                  ', 0, 1, 'C');
    ...
    $pdf->MultiCell(49, 7, $header['Description'], 0, 'L');
    $pdf->Ln(5);
    ...
    while ($row = mysqli_fetch_array($resultDetail)) {
        ...
        if($header["CategoryCD"] == "BB"){
            ...
            $nameLength = strlen($row['GoodsName']);
            if ($nameLength > $maxNameLength) {
                $lineHeightUsed = $lineHeightE;
            } else {
                $lineHeightUsed = $lineHeight;
            ...
            $nameLength = strlen($row['PartName']);
            if ($nameLength > $maxNameLength) {
                $lineHeightUsed = $lineHeightE;
            } else {
                $lineHeightUsed = $lineHeight;
        ...
        $pdf->MultiCell(62, $lineHeightUsed, $itemname, 0, 'L');
        ...
        $pdf->Cell(10, 10, $row['Quantity_2'] != NULL ? number_format($row['Quantity_2'], 0):'-', 0, 0, 'R');
        $pdf->Cell(55, 10, $row['UnitCD_2'] != NULL ? $row['UnitCD_2']:'-', 0, 1, 'C');
