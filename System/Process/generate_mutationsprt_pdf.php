...
if (isset($_GET['MutationID'])) {
    ...
    SELECT 
        md.MutationID, 
        p.PartName AS NamaMaterial, 
        md.FlowIn AS Masuk, 
        md.FlowOut AS Keluar, 
        md.UnitCD AS Satuan, 
        md.Description AS Keterangan 
    FROM 
        mutationdetailsprt md
    JOIN 
        sparepart p ON md.PartCD = p.PartCD
    WHERE 
    ...
    $pdf->SetXY(120, 15);
    $pdf->SetFont('Arial', 'B', 16);
    $pdf->Cell(0, 10, 'Mutasi Sparepart', 0, 1, 'L');
    ...
    $pdf->Output('I', 'Mutasi_' . ($header['MutationID'] ?? 'Unknown') . '.pdf');
