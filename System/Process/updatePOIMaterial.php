...
$queryh = "UPDATE `importpurchaseorderheader` SET 
            `DeliveryDate` = ?, 
            `ShippingAddress` = ?, 
            `CategoryCD` = ?, 
            `SupplierNum` = ?, 
...
// Hapus detail lama
$queryDeleteDetails = "DELETE FROM `importpurchaseorderdetail` WHERE `PurchaseOrderID` = ?";
...
if ($resulth) {
    ...
    for ($i = 0; $i < count($arrMaterial); $i++) {
        ...
        $queryd = "INSERT INTO `importpurchaseorderdetail`
...
$queryu = "UPDATE importpurchaseorderheader SET TotalPurchase=? WHERE PurchaseOrderID=?";
...
if ($resulth && $resultd) {
    header("Location:../Import-Purchasing/purchasing-material.php?status=success-update");
} else {
    header("Location:../Import-Purchasing/purchasing-material.php?status=error-update");
}
