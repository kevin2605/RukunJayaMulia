...
if ($resulth) {
    ...
    if ($resultd) {
        logAction($conn, $creator, 'Create', 'menambahkan mutasi saldo awal produk', 0, $mutid);
        header("Location:../Mutation/startBalance.php?status=new-success");
    } else {
        logAction($conn, $creator, 'Create', 'gagal menambahkan mutasi saldo awal produk', 1, $mutid);
