...
$queryh = "INSERT INTO `mutationheader` (`MutationID`, `CreatedOn`, `CreatedBy`, `Description`, `CategoryCD`) 
           VALUES ('$mutid', '$datetime', '$creator', '$desc', 'SAM')";
$resulth = mysqli_query($conn, $queryh);

if ($resulth) {
    $arrMat = $_POST["materials"];
    ...
    for ($i = 0; $i < count($arrMat); $i++) {
        $matcd = $arrMat[$i];
        $flowin = isset($arrFlowin[$i]) ? $arrFlowin[$i] : 0;
        $flowout = isset($arrFlowout[$i]) ? $arrFlowout[$i] : 0;
        $unit = $arrUnit[$i];
        $description = $arrDesc[$i];
        
        $queryd = "INSERT INTO `mutationdetailmat` (`MutationID`, `CreatedOn`, `MaterialCD`, `FlowIn`, `FlowOut`, `UnitCD`, `Description`) 
                   VALUES ('$mutid', '$datetime', '$matcd', '$flowin', '$flowout', '$unit', '$description')";
        echo $queryd;
        $resultd = mysqli_query($conn, $queryd);
    }
    
    if ($resultd) {
        logAction($conn, $creator, 'Create', 'menambahkan mutasi saldo awal bahan', 0, $mutid);
        header("Location:../Mutation/startBalanceMaterial.php?status=new-success");
    } else {
        logAction($conn, $creator, 'Create', 'gagal menambahkan mutasi saldo awal bahan', 1, $mutid);
        header("Location:../Mutation/startBalanceMaterial.php?status=error");
    }
} else {
