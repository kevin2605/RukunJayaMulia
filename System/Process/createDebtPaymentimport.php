...
while ($duplicate) {
    ...
    $query = "SELECT DebtPaymentID FROM debtpaymentheaderimport WHERE DebtPaymentID = '" . $cpid . "'";
...
$queryh = "INSERT INTO `debtpaymentheaderimport`(`DebtPaymentID`, `CreatedOn`, `CreatedBy`, `SupplierNum`, `PaymentMethod`, `Description`)
...
if ($resulth == 1) {
    ...
    for ($i = 0; $i < count($arrID); $i++) {
        ...
        $queryd = "INSERT INTO `debtpaymentdetailimport`(`DebtPaymentID`, `CreatedOn`, `RCV_InvoiceID`, `TotalPayment`)
...
if ($resulth && $resultd && $resultu) {
    ...
    header("Location:../Payment/payment-of-debt-import.php?status=success");
} else {
    logAction($conn, $creator, 'Create', 'Add Pelunasan Hutang Failed', 1, $cpid);
    header("Location:../Payment/payment-of-debt-import.php?status=error");
}
