...
if (isset($_GET['MutationID'])) {
    ...
    $pdf->SetXY(15, 15);
    $pdf->Cell(0, 10, 'PT. INDOPACK MULTI PERKASA                    Mutasi Premium                    ', 0, 1, 'C');
    ...
    $pdf->SetX(130);
    $pdf->Cell(0, 7, 'Tipe Mutasi             : Premium', 0, 1);
    $pdf->SetX(130);
    $pdf->Cell(0, 7, 'Tgl. Mutasi              : ' . $header['CreatedOn'], 0, 1);
    $pdf->SetX(130);
    $pdf->Cell(29, 7, 'Keterangan             : ', 0, 0);
    $pdf->SetX(163);
    $pdf->MultiCell(45, 7, $header['Description'], 0, 'L');
    $pdf->Ln(10);
    ...
    while ($row = mysqli_fetch_array($resultDetail)) {
        ...
        $namaProduk = !empty($row['NamaProduk']) ? $row['NamaProduk'] : $row['ProductCD'];
        $pdf->MultiCell(89, $lineHeight, $namaProduk, 0, 'L');
    ...
    $pdf->Output('I', 'Mutation_' . ($header['MutationID'] ?? 'Unknown') . '.pdf');
